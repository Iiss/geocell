package mvc.models 
{
	...
	public class SessionModel extends EventDispatcher
	{
		...
		public const PLANTS_DATA_VAR:String = "plants";
		...
		private function updatePlants():void
		{
			var plants:Array = dumpToArray(PLANTS_DATA_VAR);
			var links:Array= [];
			for (var i:int = 0; i < plants.length; i++)
			{
				trace(plants[i].cell_y * _mapInfo.width + plants[i].cell_x)
				links[plants[i].cell_y * _mapInfo.width + plants[i].cell_x] = plants[i];
			}
			
			for (i = 0; i < _cells.length; i++ )
			{
				if (i < links.length)
					_cells[i].plant = links[i];
				else
					_cells[i].plant = null;
			}
